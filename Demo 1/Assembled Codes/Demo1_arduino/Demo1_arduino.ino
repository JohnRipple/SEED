...
void loop() {
    ...
    if ( millis() % 10 == 0){
      ...
      PWM_value_ML = ((barVoltage + deltaVoltage) / 2);
      //PWM_value_MR = (desAngVel != 0) ? ((barVoltage - deltaVoltage) / 2): PWM_value_ML; //MIGHT NEED TO FLIP THE PLUS AND MINUS SIGNS ON THESE DEPENDING ON WHETHER YOU SET RIGHT WHEEL AS ONE OR TWO AND VICE VERSA
      PWM_value_MR = ((barVoltage - deltaVoltage) / 2);
      ...
      if(STRAIGHT == false){ //so it only matters when it is turning??
        adjustVariable = 1;
      }else{
      ...
      //PWM_value_MR = (desAngVel > 0) ? ((barVoltage - deltaVoltage) / 2): PWM_value_ML; //MIGHT NEED TO FLIP THE PLUS AND MINUS SIGNS ON THESE DEPENDING ON WHETHER YOU SET RIGHT WHEEL AS ONE OR TWO AND VICE VERSA
      PWM_value_ML = PWM_value_ML *  adjustVariable;
      
      //MAKES SURE THE PWM IS WITHIN THE BOUNDS
      PWM_value_MR = abs(PWM_value_MR);
      if (PWM_value_MR > 255) {
        PWM_value_MR = 255;
      }
      PWM_value_ML = abs(PWM_value_ML); 
      if (PWM_value_ML > 255) {
        PWM_value_ML = 255;
      }
      ...
      int bound = 70;
      if(PWM_value_MR != 0) {
        PWM_value_MR = ((double)PWM_value_MR)/(255)*(255-bound)+bound;
        PWM_value_ML = ((double)PWM_value_ML)/(255)*(255-bound)+bound;
      } 
//      if(abs(left.read()-right.read()) > 100 && STRAIGHT) {
//        duh = true;
//      }
//      if(duh){
//        
//        if(left.read() > right.read()) {
//          PWM_value_ML = (double)PWM_value_ML*(right.read()/left.read());
//        } else {
//          PWM_value_MR = (double)PWM_value_MR*(left.read()/right.read());
//        }
//        if(abs(left.read()-right.read()) < 5) {
//          duh = false;
//        }
//      }
//      
      ...
      analogWrite(M1Speed, PWM_value_MR );//PWM_value, WHEEL ON RIGHT SIDE, could adjust by multiplying 0.94, * blah
      digitalWrite(M2Dir, DIRECTIONM2);
      analogWrite(M2Speed, PWM_value_ML);//PWM_value, WHEEL ON LEFT SIDE
...
//        Serial.print('\t');
        ...
        Serial.print('\t');
        Serial.print(PWM_value_MR);
        Serial.print('\t');
        Serial.print(PWM_value_ML);
        Serial.print('\t');
